...
struct damon_access_report {
	 ...
	 * ring.  Must not be NULL for PA-only perf-event reports.
	 */
...
/**
 * struct damon_perf_event_attr - PMU event attributes for a perf-event probe.
 *
 * A subset of perf_event_attr chosen at probe creation time to select the
 * PMU (AMD IBS, Intel PEBS, ...) and its sampling parameters.  Used both in
 * &struct damon_prep (the userspace-configured request) and in &struct
 * damon_perf_probe_event (the armed descriptor), so the two are copied with
...
struct damon_prep {
	...
	struct damon_perf_event_attr perf;
/* private: */
	/* siblings list. */
	struct list_head list;
};
...
struct damon_probe {
	unsigned int weight;
	bool event_driven;	/* ring drain is primary; apply_probes also credits */
