		...
		struct perf_sample_data *data, struct pt_regs *regs)
{
	...
	/*
	 * tid and tgid are intentionally unset (zero-initialized).  paddr
	 * ctxs do not target a pid -- kdamond_apply_access_report() ignores
	 * report->tgid for non-pid targets -- so leaving them zero saves a
	 * pair of accesses to current->{pid,tgid} in the NMI hot path.
	 */
	report = (struct damon_access_report){
...
static int damon_perf_cpu_online(unsigned int cpu, struct hlist_node *node)
{
	...
	if (IS_ERR(perf_event)) {
		long err = PTR_ERR(perf_event);

		pr_warn_ratelimited("damon-perf: cpu %u event create failed: %ld\n",
				cpu, err);
		trace_damon_perf_cpu_init_failed(cpu, event->attr.type,
