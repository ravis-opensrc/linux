		...
		struct damon_probe *probe, struct damon_ctx *ctx)
{
	...
	/*
	 * Event-driven probes (perf-event/PEBS/IBS/HMU) have no software prep
	 * action: their hits arrive asynchronously through the per-CPU report
	 * rings and are applied by the kdamond ring drain, so skip the software
	 * prep path here.  Reaching this for an event-driven probe is normal
	 * (prep runs for every probe); it is silently skipped, not a warning.
	 */
	...
	damon_for_each_target(t, ctx) {
		damon_for_each_region(r, t) {
			...
			damon_for_each_probe(p, ctx) {
				 ...
				 */
				if (!p->event_driven &&
				    damon_pa_filter_pass(pa, folio, p))
					r->probe_hits[i]++;
				i++;
			}
			if (folio)
