...
damon_sysfs_perf_event_attr_alloc(void)
{
	...
	/*
	 * precise_ip defaults to 0 (left zero by kzalloc) so a fresh event
	 * works on AMD IBS Op without userspace tweaks.  Intel PEBS users
	 * override to 2 ("constant skid") via sysfs.
	 */
	return attr;
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf, "%c\n", perf_event_attr->sample_phys_addr ? 'Y' : 'N');
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf, "%c\n", perf_event_attr->sample_weight_struct ? 'Y' : 'N');
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf, "%c\n", perf_event_attr->exclude_kernel ? 'Y' : 'N');
}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sysfs_emit(buf, "%c\n", perf_event_attr->exclude_hv ? 'Y' : 'N');
}
		...
		struct damon_sysfs_sample *sysfs_sample)
{
	...
	struct damon_perf_event *e, *next;
	int i, err;
	...
	/*
	 * Defensive: ctx->perf_events is expected to be empty here because
	 * damon_sysfs_apply_inputs() is only called from damon_sysfs_build_ctx()
	 * on a freshly-allocated ctx.  Drain anyway so that a future refactor
	 * that reuses the ctx does not silently duplicate entries.  The events
	 * have not been initialized yet (priv == NULL); list_del + kfree is
	 * sufficient.
	 */
	list_for_each_entry_safe(e, next, &ctx->perf_events, list) {
		list_del(&e->list);
		kfree(e);
	}
