				...
				struct pt_regs *regs)
{
	 ...
	 *
	 * The refcount must be acquired BEFORE loading ctx: teardown clears
	 * ctx and then drops the armed reference.  If we loaded ctx first,
	 * teardown could free the descriptor between the load and the
	...
	/*
	 * TID/TGID are read from data->tid_entry which perf core fills before
	 * calling the overflow handler.  This avoids calling task_pid_vnr()
	 * directly from the handler context.
	 */
	if (data->sample_flags & PERF_SAMPLE_TID) {
		report.tid  = data->tid_entry.tid;
		report.tgid = data->tid_entry.pid;  /* pid_entry.pid = tgid */
	}
			       ...
			       struct damon_perf_probe_event *event)
{
	 ...
	 *
	 * The perf state must stay valid until the drain completes: a
	 * handler holding a reference has a cached perf pointer and will
	 * touch perf->accepted/dropped after we cleared ctx.
	 */
	refcount_dec(&event->refcount);
 ...
 *
 * Transactional: if the new event fails to arm, the old event's attributes
 * are used to restore the previous arming.  Only if restoration also fails
 * is nothing left armed (the caller should then clear the probe's
 * event_driven flag).
 *
 * Returns 0 on success (new event armed), 1 if the old event was restored
 * after the new event failed, negative errno on failure.
 */
			   ...
			   struct damon_perf_probe_event *new)
{
	...
	if (new->attr.type == old_pmu_type) {
		 ...
		 */
		owner_held = damon_pmu_owner_get(ctx, old_pmu_type);
		damon_perf_probe_teardown(ctx, old);
		err = damon_perf_probe_setup(ctx, probe, new);
		if (owner_held)
			damon_pmu_owner_put(ctx, old_pmu_type);
	} else {
		/*
	...
	if (err) {
		/*
		 * New event failed to arm: try to restore the old arming
		 * from the saved attributes.  Allocate a fresh descriptor
		 * since teardown freed the original.  On success the
		 * restored descriptor is attached to the probe; the caller
		 * must not overwrite it with the failed @new.
		 */
		struct damon_perf_probe_event *restore;

		restore = kzalloc(sizeof(*restore), GFP_KERNEL);
		if (restore) {
			restore->attr = old_attr;
			if (!damon_perf_probe_setup(ctx, probe, restore)) {
				/* Restoration succeeded; ownership to probe. */
				probe->perf_priv = restore;
				return 1;
			}
			kfree(restore);
		}
